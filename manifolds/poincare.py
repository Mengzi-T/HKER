"""Poincare ball manifold. Original version copied from https://github.com/HazyResearch/hgcn """
import torch
...
import utils.gyrovector_utils as gyro
...
class PoincareBall(Manifold):
    ...
    def proj(self, x, c):
        ...
        maxnorm = (1 - self.eps[str(x.dtype)]) / (c ** 0.5)
        cond = norm > maxnorm
    ...
    def weighted_midpoint(
        ...
        return gyro.weighted_midpoint(xs, k, weights, reducedim, dim, keepdim, lincomb, posweight, coadd)
    ...
    @jit.script_method
    def sqdist_for_mat(self, x: torch.Tensor, y: torch.Tensor, c: torch.Tensor, square: bool=True):
        r"""The matrix version of sqdist to accelerate computation and save memory.
        """
        sqrt_c = c ** 0.5
        x2 = x.pow(2).sum(dim=1, keepdim=True)  # B x 1
        y2 = y.pow(2).sum(dim=0, keepdim=True)  # 1 x I
        xy = -x @ y                              # B x I
        coef_x = 1 + 2 * c * xy + c * y2    # B x I
        coef_y = 1 - c * x2 # B x 1
        num = coef_x ** 2 * x2 + coef_y ** 2 @ y2 + 2 * coef_x * coef_y * xy # B x I
        denom = 1 + 2 * c * xy + c ** 2 * x2 @ y2 # B x I
        dist_c = self.artanh(
            sqrt_c * num.sqrt() / denom.clamp_min(self.min_norm)
        )
        dist = dist_c * 2 / sqrt_c
        if square:
            return dist ** 2
