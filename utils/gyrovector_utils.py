...
from .other_utils import clamp_abs, drop_dims, list_range, sabs, sign
...
def _mobius_matvec(m: torch.Tensor, x: torch.Tensor, k: torch.Tensor, dim: int = -1):
    ...
    x_norm = x.norm(dim=dim, keepdim=True, p=2).clamp_min(1e-15)
    if dim != -1 or m.dim() == 2:
        # mx = torch.tensordot(x, m, [dim], [1])    # for torch 1.8
        mx = torch.tensordot(x, m, dims=([dim], [1])) # 兼容 torch 1.11 / 2.x
...
def _weighted_midpoint(
    ...
    if lincomb:
        ...
        # mobius_scalar_mul at once
        a_mean = _mobius_scalar_mul(alpha / 2, a_mean, k=k, dim=dim)
    if not keepdim:
...
def weighted_midpoint_spmm(
    ...
    return _weighted_midpoint_spmm(
        xs=xs,
        k=k,
        weights=weights,
        lincomb=lincomb,
