...
train_item_set = defaultdict(set)
...
def remap_item(train_data, test_data):
    ...
    for u_id, i_id in train_data:
        ...
        train_item_set[int(i_id)].add(int(u_id))
...
def build_adj_train(relation_dict):
    def _si_norm_lap(adj):
        # D^{-1}A
        rowsum = np.array(adj.sum(1))
    ...
    for r_id in tqdm(relation_dict.keys()):
        if r_id == 0:
            np_mat = np.array(relation_dict[r_id])
            user_indices = np_mat[:, 0]  # user_id
            item_indices = np_mat[:, 1]  # item_id (already in [0, n_items) range)

            # Values for the sparse matrix
            vals = [1.] * len(np_mat)

            adj_train= sp.coo_matrix((vals, (user_indices, item_indices)), shape=(n_users, n_items)).astype(np.float32)
            
            # normalizing the adj matrix
            mean_mat_list = _si_norm_lap(adj_train)
...
def load_data(model_args):
    ...
    adj_train = build_adj_train(relation_dict)
    ...
    return train_cf, test_cf, user_dict, n_params, graph, \
           [adj_mat_list, mean_mat_list],adj_train
